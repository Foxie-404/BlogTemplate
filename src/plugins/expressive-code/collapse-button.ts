import { definePlugin } from "@expressive-code/core";
import type { Element } from "hast";

export function pluginCollapseButton() {
    return definePlugin({
        name: "Collapse Button",
        hooks: {
            postprocessRenderedBlock: (context) => {
                // If the code block has a title, we don't add the collapse button
                // as it might conflict with the title bar layout
                const classNames = (context.renderData.blockAst.properties?.className as string[]) || [];
                if (classNames.includes("has-title")) {
                    return;
                }

                function processCodeBlock(node: Element) {
                    // Add classes to the root node to indicate it's collapsible and expanded by default
                    if (!node.properties) node.properties = {};
                    const classNames = (node.properties.className as string[]) || [];
                    if (!classNames.includes("collapsible")) {
                        classNames.push("collapsible");
                    }
                    if (!classNames.includes("expanded")) {
                        classNames.push("expanded");
                    }
                    node.properties.className = classNames;

                    const collapseButton = {
                        type: "element" as const,
                        tagName: "button",
                        properties: {
                            className: ["collapse-btn"],
                            "aria-label": "Collapse code",
                            type: "button",
                        },
                        children: [
                            {
                                type: "element" as const,
                                tagName: "svg",
                                properties: {
                                    viewBox: "0 0 24 24",
                                    xmlns: "http://www.w3.org/2000/svg",
                                    className: ["collapse-icon"],
                                },
                                children: [
                                    {
                                        type: "element" as const,
                                        tagName: "g",
                                        properties: { fill: "none" },
                                        children: [
                                            {
                                                type: "element" as const,
                                                tagName: "path",
                                                properties: { d: "M24 0v24H0V0zM12.593 23.258l-.011.002l-.071.035l-.02.004l-.014-.004l-.071-.035q-.016-.005-.024.005l-.004.01l-.017.428l.005.02l.01.013l.104.074l.015.004l.012-.004l.104-.074l.012-.016l.004-.017l-.017-.427q-.004-.016-.017-.018m.265-.113l-.013.002l-.185.093l-.01.01l-.003.011l.018.43l.005.012l.008.007l.201.093q.019.005.029-.008l.004-.014l-.034-.614q-.005-.019-.02-.022m-.715.002a.02.02 0 0 0-.027.006l-.006.014l-.034.614q.001.018.017.024l.015-.002l.201-.093l.01-.008l.004-.011l.017-.43l-.003-.012l-.01-.01z" },
                                                children: [],
                                            },
                                            {
                                                type: "element" as const,
                                                tagName: "path",
                                                properties: {
                                                    d: "m12 16.172l-4.95-4.95a1 1 0 1 0-1.414 1.414l5.657 5.657a1 1 0 0 0 1.414 0l5.657-5.657a1 1 0 0 0-1.414-1.414z",
                                                },
                                                children: [],
                                            },
                                        ],
                                    },
                                ],
                            },
                        ],
                    } as Element;

                    if (!node.children) {
                        node.children = [];
                    }
                    
                    // We need to add the button to the frame, but expressive-code's postprocessRenderedBlock 
                    // context.renderData.blockAst is the root of the rendered block.
                    // The frame is usually a child of this.
                    node.children.push(collapseButton);
                }

                processCodeBlock(context.renderData.blockAst);
            },
        },
    });
}
